...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        print("**Evaluating...**")
        if args.dataset == 'Inshop':
            F1, NMI, recall_all_k, MAP, RP = utils.evaluate_cos_Inshop(model, dl_query, dl_gallery)

        elif args.dataset != 'SOP':
            acc, F1, NMI, recall_all_k, MAP, RP = utils.evaluate_cos(model, dl_ev)

    t = datetime.fromtimestamp(time.time()).strftime("%Y%m%d_%H%M%S")
    with open(f'test_result_{t}.txt', 'w') as f:
        f.write(f'Accuracy: {acc*100:.4f}% \nF1: {F1:.4f} \nNMI: {NMI:.4f} \nrecall@1: {recall_all_k[0]:.4f} \n'
                f'recall@2: {recall_all_k[1]:.4f} \nrecall@3: {recall_all_k[2]:.4f} \nrecall@4: {recall_all_k[1]:.4f} \n'
                f'MAP: {MAP:.4f} \nRP: {RP:.4f}')
    f.close()
