    ...
    for epoch in range(0, args.nb_epochs):
        ...
        if args.warm > 0:
            if args.gpu_id != -1:
                ...
                unfreeze_model_param = list(criterion.parameters())
                for i in range(12):
                    unfreeze_model_param = unfreeze_model_param + list(
                        model.module.blocks[i].attn.gate.parameters()) + list(model.module.blocks[i].gate.parameters())
        ...
        for batch_idx, (x, y) in pbar:
            m1, m2, x_ori_d_attn, x_hard_d_attn, x_ori_d_mlp, x_hard_d_mlp = model(x.squeeze().cuda())
        ...
        if epoch >= 0:
            with torch.no_grad():
                ...
                if args.dataset == 'Inshop':
                    ...
                    acc, F1, NMI, Recalls, MAP, RP = utils.evaluate_cos(model, dl_ev)
