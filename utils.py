...
def f1_score(query_labels, cluster_labels):
    ...
    # compute tp
    tp = sum([sum([comb(item, 2) for item in
                   np.unique(cluster_labels[query_labels == query_label], return_counts=True)[1] if item > 1]) for
              query_label in qlabels_set])
...
def calculate_mean_average_precision_at_r(knn_labels, query_labels, not_lone_query_mask, embeds_same_source,
                                          label_counts):
...
def recall_at_k(knn_labels, gt_labels, k):
    accuracy_per_sample = np.array(
        [float(gt_label in recalled_predictions[:k]) for gt_label, recalled_predictions in zip(gt_labels, knn_labels)])
    return np.mean(accuracy_per_sample)
...
def evaluate_cos(model, dataloader):
    ...
    with torch.no_grad():
        ...
        cluster_labels = run_kmeans(feature_coll, n_classes, None) + 100
        acc = accuracy_score(target_labels, cluster_labels)
    ...
    return acc, F1, NMI, recall_all_k, MAP, RP
